...
function updatePost(params){
  if(params){
    var defer = q.defer();
    var query = conn.query('UPDATE  posts SET title = ?,content = ?,auther = ? , updated_at=? WHERE id = ?',[params.title,params.content,params.auther,new Date(),params.id],function(err,result){
        if(err){
            defer.reject(err);
        }
        else{
